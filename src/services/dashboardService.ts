...
export class DashboardService {
    ...
    async manageModels(): Promise<void> {
        ...
        const profileNames = profiles.map((p: ModelProfile) => p.name);
        ...
        if (selectedProfile) {
            const profile = profiles.find((p: ModelProfile) => p.name === selectedProfile);
            if (profile) {
    ...
    private async showModelActions(profile: ModelProfile): Promise<void> {
        const actions = [
            'Edit Profile',
            'Delete Profile',
            'Clone Profile',
    ...
    private async editModelProfile(profile: ModelProfile): Promise<void> {
    ...
    private async deleteModelProfile(profile: ModelProfile): Promise<void> {
        const confirm = vscode.workspace.getConfiguration('tokenTracker').get<boolean>('confirmBeforeDelete', true);
        
        if (confirm) {
            const result = await vscode.window.showWarningMessage(
                `Are you sure you want to delete model profile: ${profile.name}?`,
                'Yes',
                'No'
            );
    ...
    private async cloneModelProfile(profile: ModelProfile): Promise<void> {
    ...
    private async activateModelProfile(profile: ModelProfile): Promise<void> {
