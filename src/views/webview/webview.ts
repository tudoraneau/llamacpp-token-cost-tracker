...
// This file will be compiled to webview.js
// Webview script for the dashboard

// Declare the VS Code webview API
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (dashboard) {
        ...
        const outputCost = document.getElementById('output-cost') as HTMLInputElement;
...
function handleUpdateCost() {
    const inputCostEl = document.getElementById('input-cost') as HTMLInputElement;
    const outputCostEl = document.getElementById('output-cost') as HTMLInputElement;
...
function handleUpdateProxySettings() {
    const proxyPortEl = document.getElementById('proxy-port') as HTMLInputElement;
    const proxyTargetUrlEl = document.getElementById('proxy-target-url') as HTMLInputElement;
    ...
    switch (message.command) {
        case 'updateStats':
            ...
            if (message.sessionStats) {
                ...
                if (promptEl) promptEl.textContent = String(message.sessionStats.promptTokens || '0');
                if (completionEl) completionEl.textContent = String(message.sessionStats.completionTokens || '0');
                if (totalEl) totalEl.textContent = String(message.sessionStats.totalTokens || '0');
                if (costEl) costEl.textContent = '$' + (message.sessionStats.cost || 0).toFixed(2);
            }
            if (message.lifetimeStats) {
                ...
                if (lifetimeTokensEl) lifetimeTokensEl.textContent = String(message.lifetimeStats.totalTokens || '0');
                if (lifetimeCostEl) lifetimeCostEl.textContent = '$' + (message.lifetimeStats.cost || 0).toFixed(2);
            }
            if (message.costSettings) {
                const inputCostEl = document.getElementById('input-cost') as HTMLInputElement;
                const outputCostEl = document.getElementById('output-cost') as HTMLInputElement;
                
                if (inputCostEl) inputCostEl.value = String(message.costSettings.inputCostPerMillion || '0');
                if (outputCostEl) outputCostEl.value = String(message.costSettings.outputCostPerMillion || '0');
            }
            if (message.serverUrl !== undefined) {
                const serverUrlEl = document.getElementById('server-url') as HTMLInputElement;
                
                if (serverUrlEl) serverUrlEl.value = message.serverUrl || '';
            }
